...
Per minted `claim_proof`, the proxy stores only the minimum replay state needed for MVP:

- a verifier derived from the proof
- the bound `installation_id`
- `expires_at`
- whether that proof has already been consumed successfully

## Hosted Setup Callback

The MVP hosted setup flow mints the first `claim_proof` directly from the GitHub App setup callback after GitHub has already identified the installation.

### `GET /setup/install/callback`

Request query:

- `installation_id`: required positive GitHub App installation id from the GitHub setup/install redirect
...
- the server mints a fresh `claim_proof` every time this callback succeeds
- each proof is bound to exactly one `installation_id`
- each proof expires 10 minutes after issuance
- the callback persists the proof verifier and expiry immediately so restarts still reject replay

For CLI issue `#4`, this callback is the documented source of the initial `claim_proof` during hosted-proxy setup. The CLI should treat the callback response as the server-issued bootstrap credential and must not invent or derive its own proof client-side.
...
- neither auth fields present: `400 Bad Request`
