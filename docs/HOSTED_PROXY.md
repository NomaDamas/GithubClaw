...
The durable store lives at `~/.githubclaw/hosted_proxy_state.json`.
...
- secrets are not stored in plaintext; the file stores verifiers
- `consumed_claim_proofs` is persisted so proof replay is still rejected after restart
- future format changes must bump `version`
