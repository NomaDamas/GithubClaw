...
For MVP, hosted-proxy registration state is persisted in:

```text
~/.githubclaw/hosted_proxy/state.json
```

The file is server-owned JSON and is replaced by writing a temp file in the same directory and renaming it into place. That gives MVP-level crash safety for claim/update writes without introducing a database.
...
```json
{
  ...
  "installations": {
    "123456": {
      ...
      "update_secret_salt": "a7b1...",
      "update_secret_hash": "5d3c...",
      "created_at": "2026-03-12T01:23:45Z",
      "updated_at": "2026-03-12T01:23:45Z",
      "last_claim_proof_fingerprint": "9f0e...",
      "last_claim_proof_issued_at": "2026-03-12T01:20:00Z",
      "last_claimed_at": "2026-03-12T01:23:45Z"
    }
  },
  "used_claim_proofs": {
    "9f0e...": {
      "installation_id": 123456,
      "fingerprint": "9f0e...",
      "issued_at": "2026-03-12T01:20:00Z",
      "consumed_at": "2026-03-12T01:23:45Z"
    }
  }
}
```
...
- `tunnel_url` is stored only in canonical normalized form.
- `update_secret` itself is not persisted directly; MVP stores salted verifier material instead.
- claim replay state persists enough metadata to reject reused proofs after restart.
