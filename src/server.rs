...
use crate::registration::{RegistrationError, RegistrationOutcome, RegistrationState};
...
async fn register_handler(
    ...
    if request.installation_id == 0 {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({
                "error": "installation_id must be greater than 0"
            })),
        ));
    }
        ...
        .await
    {
        Ok(outcome) => {
            ...
            let tunnel_url = state
                ...
                .await
                .map(|registration| registration.tunnel_url)
                .unwrap_or_else(|| request.tunnel_url.trim().to_string());
        ...
        Err(error) => {
            let status = match error {
                ...
                | RegistrationError::InvalidTokenSignature
                | RegistrationError::ExpiredToken
                | RegistrationError::InstallationMismatch => StatusCode::FORBIDDEN,
                RegistrationError::Persistence(_) => StatusCode::INTERNAL_SERVER_ERROR,
            };
            Err((
                status,
                Json(serde_json::json!({
...
mod tests {
    ...
    fn make_test_state(tmp: &TempDir) -> Arc<ServerState> {
        ...
        Arc::new(ServerState {
            ...
            registration: RegistrationState::new_for_tests(
                tmp.path().join(".githubclaw").join("installations.json"),
    ...
    fn registration_token(installation_id: u64) -> String {
        crate::registration::mint_registration_token(
            &crate::registration::RegistrationClaims {
                installation_id,
    ...
    #[tokio::test]
    async fn test_register_creates_installation_mapping() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
                ...
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&serde_json::json!({
                            ...
                            "registration_token": registration_token(42),
                        }))
                        .unwrap(),
                    ))
                    .unwrap(),
    ...
    #[tokio::test]
    async fn test_register_rejects_unsafe_tunnel_url() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let app = create_router(state);
    ...
    #[tokio::test]
    async fn test_register_rejects_installation_mismatch() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let app = create_router(state);
            ...
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&serde_json::json!({
                            "installation_id": 43,
                            "tunnel_url": "https://good.trycloudflare.com",
                            "registration_token": registration_token(42),
                        }))
                        .unwrap(),
                    ))
    ...
    #[tokio::test]
    async fn test_register_updates_existing_installation_mapping() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let app = create_router(Arc::clone(&state));
        ...
        let stored = state.registration.get(42).await.unwrap();
        assert_eq!(stored.tunnel_url, "https://second.trycloudflare.com");
    }
