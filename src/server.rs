...
use crate::hosted_proxy::{normalize_tunnel_url, HostedProxyStore, TunnelUrlError};
...
pub struct ServerState {
    ...
    pub hosted_proxy: Mutex<HostedProxyStore>,
...
fn error_response(code: &str, message: impl Into<String>) -> Json<Value> {
...
async fn hosted_setup_handler(
    State(state): State<Arc<ServerState>>,
    Query(query): Query<HostedSetupQuery>,
    ...
    let minted = state
        ...
        .lock()
        .await
        .mint_claim_proof(query.installation_id, now)
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                error_response("persistence_failure", e),
            )
        })?;
    ...
    Ok((
        ...
        Json(serde_json::json!({
            ...
            "claim_proof": minted.claim_proof,
...
async fn register_handler(
    ...
    let has_claim_proof = payload.claim_proof.is_some();
    let has_update_secret = payload.update_secret.is_some();
    ...
    let tunnel_url = normalize_tunnel_url(&payload.tunnel_url).map_err(|err| match err {
        TunnelUrlError::InvalidTunnelUrl => (
            StatusCode::BAD_REQUEST,
            error_response(
                "invalid_tunnel_url",
        ...
        TunnelUrlError::UnsafeTunnelUrl => (
            StatusCode::BAD_REQUEST,
            error_response(
                "unsafe_tunnel_url",
                "tunnel_url host must not target localhost, loopback, or private network ranges.",
            ),
        ),
    })?;
    ...
    let now = Utc::now();
    let result = {
        let mut store = state.hosted_proxy.lock().await;
        if let Some(claim_proof) = payload.claim_proof.as_deref() {
            store.register_with_claim_proof(payload.installation_id, tunnel_url, claim_proof, now)
        } else {
            store.register_with_update_secret(
                payload.installation_id,
                tunnel_url,
                payload.update_secret.as_deref().unwrap(),
                now,
            )
        }
    };
    ...
    match result {
        Ok(success) => Ok((
            StatusCode::from_u16(success.status_code).unwrap(),
            Json(serde_json::json!({
...
pub fn create_router(state: Arc<ServerState>) -> Router {
    Router::new()
        ...
        .route("/hosted/setup", get(hosted_setup_handler))
...
mod tests {
    ...
    fn make_test_state(tmp: &TempDir) -> Arc<ServerState> {
        ...
        Arc::new(ServerState {
            ...
            hosted_proxy: Mutex::new(HostedProxyStore::load_or_default(tmp.path()).unwrap()),
    ...
    #[tokio::test]
    async fn test_hosted_setup_mints_claim_proof_for_installation() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/hosted/setup?installation_id=123456")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let body: Value = serde_json::from_str(&body_string(response).await).unwrap();
        let claim_proof = body["claim_proof"].as_str().unwrap();
        assert!(claim_proof.starts_with("cp_"));
        assert_eq!(body["installation_id"], 123456);

        let store = state.hosted_proxy.lock().await;
        let proof = store.state().proofs.get(claim_proof).unwrap();
        assert_eq!(proof.installation_id, 123456);
        assert!(proof.used_at.is_none());
    }

    #[tokio::test]
    async fn test_register_claim_succeeds_and_replay_fails() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let app = create_router(state);
        ...
        let setup_response = app
            ...
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/hosted/setup?installation_id=777")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
            .unwrap();
        let setup_body: Value = serde_json::from_str(&body_string(setup_response).await).unwrap();
        let claim_proof = setup_body["claim_proof"].as_str().unwrap().to_string();
        ...
        let claimed_body: Value = serde_json::from_str(&body_string(claimed).await).unwrap();
        assert_eq!(claimed_body["status"], "claimed");
        assert_eq!(claimed_body["rotated"], false);
        assert!(claimed_body["update_secret"]
            .as_str()
            .unwrap()
            .starts_with("us_"));
        ...
        let replay_body: Value = serde_json::from_str(&body_string(replay).await).unwrap();
        assert_eq!(replay_body["error"]["code"], "already_claimed");
    }

    #[tokio::test]
    async fn test_register_update_rotates_secret() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let app = create_router(state);
        ...
        let setup_response = app
            ...
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/hosted/setup?installation_id=888")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        let original_secret = claim_body["update_secret"].as_str().unwrap().to_string();
            ...
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&serde_json::json!({
                            "installation_id": 888,
                            "tunnel_url": "https://next.example.com:8443",
                            "update_secret": original_secret,
                        }))
                        .unwrap(),
                    ))
                    .unwrap(),
        ...
        let replacement_secret = update_body["update_secret"].as_str().unwrap();
        assert_eq!(update_body["status"], "updated");
        assert_eq!(update_body["rotated"], true);
        assert_ne!(
            replacement_secret,
            claim_body["update_secret"].as_str().unwrap()
        );
        assert_eq!(update_body["tunnel_url"], "https://next.example.com:8443");

        let stale_secret_response = app
            .oneshot(
                Request::builder()
                    .method("POST")
                    .uri("/register")
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&serde_json::json!({
                            "installation_id": 888,
                            "tunnel_url": "https://final.example.com",
                            "update_secret": claim_body["update_secret"].as_str().unwrap(),
                        }))
                        .unwrap(),
                    ))
                    .unwrap(),
        ...
        assert_eq!(stale_secret_response.status(), StatusCode::FORBIDDEN);
        let stale_body: Value =
            serde_json::from_str(&body_string(stale_secret_response).await).unwrap();
        assert_eq!(stale_body["error"]["code"], "invalid_update_secret");
    }

    #[tokio::test]
    async fn test_register_rejects_unsafe_tunnel_url_without_consuming_proof() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let app = create_router(state);
        ...
        let setup_response = app
            ...
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/hosted/setup?installation_id=999")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        let bad_body: Value = serde_json::from_str(&body_string(bad_response).await).unwrap();
        assert_eq!(bad_body["error"]["code"], "unsafe_tunnel_url");
            ...
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&serde_json::json!({
                            "installation_id": 999,
                            "tunnel_url": "https://valid.example.com",
                            "claim_proof": setup_body["claim_proof"].as_str().unwrap(),
                        }))
                        .unwrap(),
                    ))
                    .unwrap(),
