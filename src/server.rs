...
use crate::hosted_proxy::{
    ClaimProofIssued, ErrorBody, HostedProxyStore, RegisterRequest, RegisterSuccess,
    SetupClaimProofQuery,
};
...
async fn setup_claim_proof_handler(
    State(state): State<Arc<ServerState>>,
    Query(query): Query<SetupClaimProofQuery>,
) -> (StatusCode, Json<Value>) {
    let mut store = state.hosted_proxy_store.lock().await;
    match store.issue_claim_proof_with_now(query.installation_id, chrono::Utc::now()) {
        Ok(issued) => setup_claim_proof_response(issued),
...
fn setup_claim_proof_response(issued: ClaimProofIssued) -> (StatusCode, Json<Value>) {
    (
        StatusCode::OK,
        Json(serde_json::json!({
...
fn success_response(success: RegisterSuccess) -> (StatusCode, Json<Value>) {
    ...
    (
        status,
        Json(serde_json::to_value(success).expect("register success serializes")),
    )
}
    ...
    (
        ...
        Json(
            serde_json::to_value(ErrorBody {
                error: crate::hosted_proxy::ErrorDetail {
                    code,
                    message: message.into(),
                },
            })
            .expect("register error serializes"),
        ),
    )
}
...
pub fn create_router(state: Arc<ServerState>) -> Router {
    Router::new()
        ...
        .route("/setup/claim-proof", get(setup_claim_proof_handler))
...
mod tests {
    ...
    #[tokio::test]
    async fn test_setup_claim_proof_returns_bound_proof_and_expiry() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let app = create_router(state);
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/setup/claim-proof?installation_id=123")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        assert_eq!(body["register_url"], "/register");
        assert!(body["claim_proof"].as_str().unwrap().starts_with("cp_v1."));
        assert!(body["expires_at"].as_str().is_some());
    }

    #[tokio::test]
    async fn test_register_claim_replay_and_installation_binding() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let app = create_router(state.clone());
        let claim_proof = {
            let response = app
                ...
                .unwrap();
            let body: Value = serde_json::from_str(&body_string(response).await).unwrap();
            body["claim_proof"].as_str().unwrap().to_string()
        };
        ...
        let claimed: Value = serde_json::from_str(&body_string(first_response).await).unwrap();
        let update_secret = claimed["update_secret"].as_str().unwrap().to_string();
        ...
        let mismatch_proof = {
            let response = app
                ...
                .oneshot(
                    Request::builder()
                        .method("GET")
                        .uri("/setup/claim-proof?installation_id=999")
                        .body(Body::empty())
                        .unwrap(),
                )
                .await
            ...
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&serde_json::json!({
                            "installation_id": 123,
                            "tunnel_url": "https://three.example.com",
                            "claim_proof": mismatch_proof,
                        }))
                        .unwrap(),
                    ))
                    .unwrap(),
        ...
        let mismatch_body: Value =
            serde_json::from_str(&body_string(mismatch_response).await).unwrap();
            ...
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&serde_json::json!({
                            "installation_id": 123,
                            "tunnel_url": "https://four.example.com",
