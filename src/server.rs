...
pub struct ServerState {
    ...
    pub hosted_proxy: Arc<crate::hosted_proxy::HostedProxyService>,
}
...
async fn register_handler(
    ...
    let request = match serde_json::from_slice::<crate::hosted_proxy::RegisterRequest>(&body) {
        Ok(request) => request,
        Err(_) => {
    ...
    match state.hosted_proxy.register(request, chrono::Utc::now()) {
        Ok(response) => {
            let status = match response.status {
                crate::hosted_proxy::RegisterStatus::Claimed => StatusCode::CREATED,
                crate::hosted_proxy::RegisterStatus::Updated => StatusCode::OK,
            };
            (status, Json(serde_json::to_value(response).unwrap()))
        }
        Err(error) => register_error_response(
            register_error_status(&error),
            error.code(),
            &error.message(),
        ),
    }
...
fn register_error_status(error: &crate::hosted_proxy::RegisterError) -> StatusCode {
    match error {
        crate::hosted_proxy::RegisterError::InvalidRequest(_)
        | crate::hosted_proxy::RegisterError::InvalidTunnelUrl
        | crate::hosted_proxy::RegisterError::UnsafeTunnelUrl => StatusCode::BAD_REQUEST,
        crate::hosted_proxy::RegisterError::InvalidClaimProof
        | crate::hosted_proxy::RegisterError::ExpiredClaimProof
        | crate::hosted_proxy::RegisterError::ClaimProofAlreadyUsed
        | crate::hosted_proxy::RegisterError::InvalidUpdateSecret
        | crate::hosted_proxy::RegisterError::OwnershipMismatch => StatusCode::FORBIDDEN,
        crate::hosted_proxy::RegisterError::AlreadyClaimed => StatusCode::CONFLICT,
        crate::hosted_proxy::RegisterError::PersistenceFailure(_)
        | crate::hosted_proxy::RegisterError::SecretRotationFailure(_) => {
            StatusCode::INTERNAL_SERVER_ERROR
        }
    }
}

fn register_error_response(
    status: StatusCode,
    code: &str,
    message: &str,
) -> (StatusCode, Json<Value>) {
...
mod tests {
    ...
    fn make_test_state(tmp: &TempDir) -> Arc<ServerState> {
        ...
        Arc::new(ServerState {
            ...
            hosted_proxy: Arc::new(crate::hosted_proxy::HostedProxyService::new(
                "test-proof-secret",
                Box::new(
                    crate::hosted_proxy::FileHostedProxyStore::new(
    ...
    #[tokio::test]
    async fn test_register_claim_and_update_round_trip() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let claim_proof = state
            .hosted_proxy
            .mint_claim_proof(123, chrono::Utc::now())
            .unwrap();
