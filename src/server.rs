...
use crate::hosted_proxy::{HostedProxyStore, RegisterRequest, SetupCallbackRequest};
...
async fn setup_handler(
    State(state): State<Arc<ServerState>>,
    query: Query<SetupCallbackRequest>,
) -> (StatusCode, Json<Value>) {
    let store = state.hosted_proxy_store.lock().await;
    match store.issue_setup_claim(query.0, chrono::Utc::now()) {
        Ok(success) => {
            let claim_proof = success.claim_proof.clone();
            (
                StatusCode::OK,
                Json(serde_json::json!({
                    "status": success.status,
                    "installation_id": success.installation_id,
                    "setup_action": success.setup_action,
                    "claim_proof": success.claim_proof,
                    "expires_at": success.expires_at,
                    "register_path": success.register_path,
                    "register_request": {
                        "installation_id": success.installation_id,
...
fn success_response(success: crate::hosted_proxy::RegisterSuccess) -> (StatusCode, Json<Value>) {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_setup_returns_claim_proof_for_installation() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
        let app = create_router(state);
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/setup?installation_id=123&setup_action=install")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        assert_eq!(body["register_request"]["installation_id"], 123);
    }

    #[tokio::test]
    async fn test_register_claim_returns_secret_and_replay_is_rejected() {
        let tmp = TempDir::new().unwrap();
        let state = make_test_state(&tmp);
