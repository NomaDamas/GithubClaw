...
#[derive(Debug, Clone, Deserialize)]
pub struct SetupCallbackRequest {
    pub installation_id: u64,
    #[serde(default)]
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct SetupCallbackSuccess {
    pub status: &'static str,
    pub installation_id: u64,
    pub setup_action: Option<String>,
...
impl HostedProxyStore {
    ...
    pub fn issue_setup_claim(
        &self,
        request: SetupCallbackRequest,
        now: DateTime<Utc>,
    ) -> Result<SetupCallbackSuccess, RegisterError> {
        ...
        let claim_proof = mint_claim_proof(&self.signing_secret, request.installation_id, now);
        Ok(SetupCallbackSuccess {
            status: "ready",
            installation_id: request.installation_id,
            setup_action: request.setup_action,
            claim_proof,
    ...
    pub fn issue_setup_claim_for_test(
        &self,
        installation_id: u64,
        setup_action: Option<String>,
        now: DateTime<Utc>,
    ) -> Result<SetupCallbackSuccess, RegisterError> {
        self.issue_setup_claim(
...
pub fn normalize_tunnel_url(raw: &str) -> Result<String, RegisterError> {
    ...
    let normalized = match url.port() {
        Some(port) if port != 443 => format!("https://{normalized_host}:{port}"),
        _ => format!("https://{normalized_host}"),
    };
...
fn mint_claim_proof(secret: &str, installation_id: u64, issued_at: DateTime<Utc>) -> String {
    ...
    let payload = format!("{CLAIM_PROOF_PREFIX}:{installation_id}:{issued_at_ts}:{nonce}");
    let signature = sign_with_secret(secret, &payload).expect("static HMAC key");
    format!("{CLAIM_PROOF_PREFIX}.{installation_id}.{issued_at_ts}.{nonce}.{signature}")
}
...
fn parse_and_verify_claim_proof(
    ...
    let installation_id = parts[1].parse::<u64>().map_err(|_| {
        RegisterError::new(
            "invalid_claim_proof",
            "Claim proof installation binding is invalid",
            403,
        )
    })?;
...
mod tests {
    ...
    #[test]
    fn test_setup_claim_issues_bound_short_lived_proof() {
        ...
        let now = DateTime::<Utc>::from_timestamp(1_700_010_000, 0).unwrap();

        let issued = store
            .issue_setup_claim_for_test(321, Some("install".to_string()), now)
            .unwrap();

        assert_eq!(issued.installation_id, 321);
        assert_eq!(issued.setup_action.as_deref(), Some("install"));
        assert_eq!(issued.register_path, "/register");
        assert_eq!(
            issued.expires_at,
    ...
    fn test_wrong_update_secret_for_other_installation_is_ownership_mismatch() {
        ...
        let proof_one = store.mint_claim_proof_for_test(1, now);
        let first = store
            .register_with_now(
                request_with_claim(1, "https://one.example.com", proof_one),
                now,
            )
            .unwrap();
        let proof_two = store.mint_claim_proof_for_test(2, now);
        store
            .register_with_now(
                request_with_claim(2, "https://two.example.com", proof_two),
                now,
            )
            .unwrap();
