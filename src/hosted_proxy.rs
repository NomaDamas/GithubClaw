...
mod tests {
    ...
    fn service(tmp: &TempDir) -> HostedProxyService {
        HostedProxyService::load(state_path(tmp)).unwrap()
    }
    ...
    #[tokio::test]
    async fn claim_proof_is_bound_to_one_installation() {
        let tmp = TempDir::new().unwrap();
        let service = service(&tmp);
        let now = Utc::now();
        let issued = service.issue_claim_proof_at(123, now).await.unwrap();
        ...
        assert_eq!(result, HostedProxyError::OwnershipMismatch);
    }
    ...
    #[tokio::test]
    async fn proof_state_persists_across_reload() {
        let tmp = TempDir::new().unwrap();
        let now = Utc::now();

        let issued_claim_proof = {
            let service = service(&tmp);
            service
                .issue_claim_proof_at(123, now)
                .await
                .unwrap()
                .claim_proof
        };

        let reloaded = HostedProxyService::load(state_path(&tmp)).unwrap();
        let claimed = reloaded
            .register_at(
                RegisterRequest {
                    installation_id: 123,
                    tunnel_url: "https://abc123.trycloudflare.com".to_string(),
                    claim_proof: Some(issued_claim_proof.clone()),
                    update_secret: None,
                },
                now + Duration::minutes(1),
            )
        ...
        let replay_service = HostedProxyService::load(state_path(&tmp)).unwrap();
        let replay = replay_service
            .register_at(
                RegisterRequest {
                    installation_id: 123,
                    tunnel_url: "https://next456.trycloudflare.com".to_string(),
                    claim_proof: Some(issued_claim_proof),
                    update_secret: None,
                },
