...
That's my decision."#;
    ...
    // 13. which lookup detects available CLI tools
    #[test]
    fn has_cli_detects_tools() {
        // We don't assert presence of claude/codex in CI, only that lookup is safe.
        let _has_claude = which::which("claude").is_ok();
        let _has_codex = which::which("codex").is_ok();
        // A definitely missing binary should return false
        assert!(which::which("definitely_not_a_real_binary_xyz").is_err());
    }
}
