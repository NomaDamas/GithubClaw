...
use crate::registration::{load_or_create_registration_secret, RegistrationState};
...
fn cmd_init() {
    ...
    // (c) One-time hosted proxy registration secret setup
    setup_registration_secret();

    // (d) Backend detection
    detect_backends();

    // (e) Pre-flight check for gh CLI
    preflight_gh();

    // (f) Guidance on what to git add
    println!();
    ...
    rt.block_on(async move {
        ...
        let registration = Arc::new(
            RegistrationState::load(
                global_dir.join("hosted_proxy").join("registration.json"),
                load_or_create_registration_secret(
                    &global_dir.join("secrets").join("registration_secret"),
                )
                .unwrap_or_else(|e| {
            ...
            .unwrap_or_else(|e| {
                eprintln!("Error loading registration state: {e}");
                std::process::exit(1);
            }),
        );
...
fn cmd_serve(host: &str, port: u16) {
    ...
    rt.block_on(async {
        ...
        let registration_secret = load_or_create_registration_secret(
            &global_dir.join("secrets").join("registration_secret"),
        )
        .unwrap_or_else(|e| {
            eprintln!("Error loading registration secret: {e}");
...
fn setup_registration_secret() {
    let secret_path = global_config_dir()
        .join("secrets")
        .join("registration_secret");
    if secret_path.exists() {
        println!("  Hosted proxy registration secret already configured.");
        return;
    }

    match load_or_create_registration_secret(&secret_path) {
        Ok(_) => println!(
            "  Generated hosted proxy registration secret at ~/.githubclaw/secrets/registration_secret"
        ),
        Err(_) => eprintln!("  Warning: could not generate hosted proxy registration secret."),
    }
}
