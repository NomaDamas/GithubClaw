...
fn cmd_bootstrap() {
    ...
    rt.block_on(async move {
        ...
        let hosted_proxy_store = crate::hosted_proxy::FileHostedProxyStore::new(
            crate::config::hosted_proxy_state_path(),
        )
        .unwrap_or_else(|e| {
            eprintln!("Failed to initialize hosted proxy state: {}", e.message());
            std::process::exit(1);
        });
        let state = Arc::new(ServerState {
            ...
            hosted_proxy: Arc::new(crate::hosted_proxy::HostedProxyService::new(
                "bootstrap-hosted-proxy-secret",
                Box::new(hosted_proxy_store),
            )),
        });
