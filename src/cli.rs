        ...
        let state = Arc::new(ServerState {
            ...
            hosted_proxy_store: Mutex::new(
                crate::hosted_proxy::HostedProxyStore::load(
                    ...
                    load_or_create_hosted_proxy_secret(),
                )
                .unwrap_or_else(|e| {
                    eprintln!("Failed to load hosted proxy store: {e}");
                    std::process::exit(1);
                }),
            ),
...
fn cmd_serve(host: &str, port: u16) {
    ...
    rt.block_on(async {
        ...
        let hosted_proxy_secret = load_or_create_hosted_proxy_secret();
        ...
        let state = Arc::new(ServerState {
            ...
            hosted_proxy_store: Mutex::new(
                ...
                .unwrap_or_else(|e| {
                    eprintln!("Error loading hosted proxy store: {e}");
                    std::process::exit(1);
                }),
            ),
...
fn load_or_create_hosted_proxy_secret() -> String {
    let secrets_dir = global_config_dir().join("secrets");
    let _ = fs::create_dir_all(&secrets_dir);
    let secret_path = secrets_dir.join("hosted_proxy_secret");
    ...
    eprintln!("Failed to create ~/.githubclaw/secrets/hosted_proxy_secret");
    std::process::exit(1);
}
