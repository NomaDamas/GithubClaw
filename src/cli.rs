...
enum Commands {
    ...
    Tui,
    /// Show a maintainer-friendly end-of-day wrap-up
    WrapUp {
        /// Render the wrap-up for a specific local date (YYYY-MM-DD)
        #[arg(long)]
        date: Option<String>,
    },
}
...
pub fn run() {
    ...
    match cli.command {
        ...
        Commands::WrapUp { date } => cmd_wrap_up(date.as_deref()),
    }
}
...
struct RepoWrapUpStats {
    repo_name: String,
    touched_issue_ids: Vec<u64>,
    active_sessions: usize,
    queue_depth: usize,
    latest_activity: Option<DateTime<Local>>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct WrapUpSummary {
    date: NaiveDate,
    repos: Vec<RepoWrapUpStats>,
    total_touched_issues: usize,
    total_active_sessions: usize,
    total_queue_depth: usize,
}

type SessionActivity = (Vec<u64>, usize, Option<DateTime<Local>>);

fn cmd_wrap_up(date: Option<&str>) {
    let date = match parse_wrap_up_date(date) {
        Ok(date) => date,
        Err(err) => {
            eprintln!("{err}");
            std::process::exit(1);
    ...
    match build_wrap_up_summary(&global_config_dir(), date) {
        Ok(summary) => {
            print!("{}", render_wrap_up(&summary));
        }
        Err(err) => {
            eprintln!("Failed to build wrap-up: {err}");
            std::process::exit(1);
        }
    }
}

fn parse_wrap_up_date(date: Option<&str>) -> Result<NaiveDate, String> {
    match date {
...
fn build_wrap_up_summary(githubclaw_home: &Path, date: NaiveDate) -> Result<WrapUpSummary, String> {
    let registry = crate::server::load_registry(&githubclaw_home.join("registry.json"));
    let session_store =
        crate::session_store::SessionStore::with_base_dir(githubclaw_home.join("sessions"));

    let mut repos: Vec<RepoWrapUpStats> = registry
        .into_iter()
        .map(|(repo_name, entry)| {
            let queue_dir = if !entry.local_path.is_empty() {
            ...
            let queue_depth = count_queue_items(&queue_dir);
            let (touched_issue_ids, active_sessions, latest_activity) =
                collect_session_activity(&session_store, &repo_name, date)
                    .map_err(|err| format!("failed reading sessions for {repo_name}: {err}"))?;
    ...
    repos.sort_by(|a, b| {
        b.touched_issue_ids
            .len()
            .cmp(&a.touched_issue_ids.len())
            .then_with(|| b.active_sessions.cmp(&a.active_sessions))
            .then_with(|| a.repo_name.cmp(&b.repo_name))
    });

    let total_touched_issues = repos.iter().map(|repo| repo.touched_issue_ids.len()).sum();
    let total_active_sessions = repos.iter().map(|repo| repo.active_sessions).sum();
    let total_queue_depth = repos.iter().map(|repo| repo.queue_depth).sum();

    Ok(WrapUpSummary {
        date,
        repos,
        total_touched_issues,
        total_active_sessions,
        total_queue_depth,
    })
}

fn count_queue_items(queue_dir: &Path) -> usize {
...
fn collect_session_activity(
    ...
    let mut touched_issue_ids = Vec::new();
    let mut latest_activity = None;

    for (issue_id, _) in &sessions {
        let issue_dir = session_store.get_session_dir(repo_name, *issue_id);
        let issue_activity = latest_mtime_in_dir(&issue_dir).map_err(|err| err.to_string())?;
        if let Some(activity) = issue_activity {
            if latest_activity.map(|ts| activity > ts).unwrap_or(true) {
                latest_activity = Some(activity);
            }
            if activity.date_naive() == date {
                touched_issue_ids.push(*issue_id);
            }
        }
    }

    touched_issue_ids.sort_unstable();
    Ok((touched_issue_ids, sessions.len(), latest_activity))
}

fn latest_mtime_in_dir(dir: &Path) -> std::io::Result<Option<DateTime<Local>>> {
    if !dir.exists() {
        return Ok(None);
    }
    ...
    for entry in fs::read_dir(dir)? {
        ...
        if latest.map(|ts| modified > ts).unwrap_or(true) {
...
fn render_wrap_up(summary: &WrapUpSummary) -> String {
    let active_repo_count = summary
        .repos
        .iter()
        .filter(|repo| {
            repo.active_sessions > 0 || repo.queue_depth > 0 || !repo.touched_issue_ids.is_empty()
        })
        .count();

    let known_work = summary.total_touched_issues + summary.total_queue_depth;
    let progress_ratio = if known_work == 0 {
        0.0
    } else {
        summary.total_touched_issues as f32 / known_work as f32
    };
    let progress_bar = render_progress_bar(progress_ratio, 20);

    let strongest_momentum = summary
        .repos
        .iter()
        .max_by_key(|repo| repo.touched_issue_ids.len());
    let most_backlogged = summary.repos.iter().max_by_key(|repo| repo.queue_depth);

    let mut out = String::new();
    out.push_str("GithubClaw Wrap-up\n");
    out.push_str(&format!("{}\n\n", summary.date.format("%Y-%m-%d")));

    out.push_str("Today at a glance\n");
    out.push_str(&format!(
        "- {} issues moved today\n",
        summary.total_touched_issues
    ));
    out.push_str(&format!("- {} repositories active\n", active_repo_count));
    out.push_str(&format!(
        "- {} live sessions still open\n",
        summary.total_active_sessions
    ));
    out.push_str(&format!(
        "- {} queued events still waiting\n\n",
        summary.total_queue_depth
    ));

    out.push_str("Progress pulse\n");
    out.push_str(&format!(
        "- {} {:>3}% of known work moved today\n",
        progress_bar,
        (progress_ratio * 100.0).round() as u32
    ));
    if summary.total_touched_issues == 0 && summary.total_queue_depth == 0 {
        out.push_str("- No tracked activity landed in the local filesystem yet\n\n");
    } else {
        out.push_str(&format!(
            "- {} issues moved, {} still queued\n\n",
            summary.total_touched_issues, summary.total_queue_depth
        ));
    }

    out.push_str("By repository\n");
    if summary.repos.is_empty() {
        out.push_str("- No repositories are registered yet\n");
    } else {
        for repo in &summary.repos {
            let issue_preview = if repo.touched_issue_ids.is_empty() {
                "none".to_string()
            } else {
                repo.touched_issue_ids
                    .iter()
                    .take(4)
                    .map(|id| format!("#{id}"))
                    .collect::<Vec<_>>()
            ...
            let last_seen = repo
                ...
                .unwrap_or_else(|| "--:--".into());
            out.push_str(&format!(
                "- {}: {} moved today | {} live | {} queued | issues: {} | last activity {}\n",
                repo.repo_name,
                repo.touched_issue_ids.len(),
    ...
    if let Some(repo) = strongest_momentum {
        if repo.touched_issue_ids.is_empty() {
            out.push_str("- Strongest momentum: nothing moved yet\n");
        } else {
            out.push_str(&format!(
                "- Strongest momentum: {} moved {} issue(s) today\n",
                repo.repo_name,
                repo.touched_issue_ids.len()
            ));
        }
    ...
    if let Some(repo) = most_backlogged {
        if repo.queue_depth > 0 {
            out.push_str(&format!(
                "- Needs attention: {} still has {} queued event(s)\n",
                repo.repo_name, repo.queue_depth
            ));
        } else {
            out.push_str("- Needs attention: queue is clear across registered repos\n");
        }
    } else {
        out.push_str("- Needs attention: no repositories registered\n");
    }

    let suggested_move = if let Some(repo) = summary.repos.iter().find(|repo| repo.queue_depth > 0)
    {
        format!("Open {} first and drain the queued work.", repo.repo_name)
    } else if let Some(repo) = summary.repos.iter().find(|repo| repo.active_sessions > 0) {
        format!(
            "Review the still-open session in {} before signing off.",
            repo.repo_name
        )
    } else {
        "No urgent follow-up is waiting in the local state snapshot.".into()
    };
    out.push_str(&format!(
...
fn render_progress_bar(ratio: f32, width: usize) -> String {
...
mod tests {
    ...
    #[test]
    fn wrap_up_date_parser_accepts_iso_date() {
        let parsed = parse_wrap_up_date(Some("2026-03-15")).unwrap();
        assert_eq!(parsed, NaiveDate::from_ymd_opt(2026, 3, 15).unwrap());
    }

    #[test]
    fn wrap_up_summary_counts_sessions_and_queue() {
        let tmp = TempDir::new().unwrap();
        let home = tmp.path().join(".githubclaw");
        let repo_root = tmp.path().join("repo");
        fs::create_dir_all(repo_root.join(".githubclaw").join("queue")).unwrap();
        ...
        fs::write(
            ...
            format!(
                "{{\"repos\":{{\"owner/repo\":{{\"local_path\":\"{}\",\"socket_path\":\"/tmp/test.sock\"}}}}}}",
                repo_root.display()
            ),
        )
        .unwrap();
        ...
        fs::write(
            repo_root
                .join(".githubclaw")
                .join("queue")
                .join("000002_issue.json"),
            "{}",
        )
        .unwrap();

        let store = crate::session_store::SessionStore::with_base_dir(home.join("sessions"));
        store.save("owner/repo", 101, "session-101").unwrap();
        store.save("owner/repo", 102, "session-102").unwrap();

        let today = Local::now().date_naive();
        let summary = build_wrap_up_summary(&home, today).unwrap();

        assert_eq!(summary.total_touched_issues, 2);
        assert_eq!(summary.total_active_sessions, 2);
        assert_eq!(summary.total_queue_depth, 2);
        assert_eq!(summary.repos.len(), 1);
        assert_eq!(summary.repos[0].touched_issue_ids, vec![101, 102]);
    }

    #[test]
    fn wrap_up_render_handles_empty_state() {
        let summary = WrapUpSummary {
            date: NaiveDate::from_ymd_opt(2026, 3, 15).unwrap(),
            repos: Vec::new(),
            total_touched_issues: 0,
            total_active_sessions: 0,
            total_queue_depth: 0,
        };

        let rendered = render_wrap_up(&summary);
        assert!(rendered.contains("GithubClaw Wrap-up"));
        assert!(rendered.contains("No repositories are registered yet"));
        assert!(rendered.contains("No tracked activity landed in the local filesystem yet"));
    }
}
