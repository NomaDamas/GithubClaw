...
fn cmd_bootstrap() {
    ...
    rt.block_on(async move {
        ...
        let registration_store = crate::server::RegistrationStore::load(
            &crate::server::registration_store_path(&global_dir),
        )
        .unwrap_or_else(|e| {
            eprintln!("Failed to load registration store: {e}");
            std::process::exit(1);
        });
        let state = Arc::new(ServerState {
            ...
            registration_store: Mutex::new(registration_store),
        });
...
fn cmd_serve(host: &str, port: u16) {
    ...
    rt.block_on(async {
        ...
        .unwrap_or_else(|e| {
            tracing::error!("Failed to load registration store: {}", e);
            std::process::exit(1);
        });
