...
fn cmd_bootstrap() {
    use crate::hosted_proxy::HostedProxyState;
    ...
    rt.block_on(async move {
        ...
        let state = Arc::new(ServerState {
            ...
            hosted_proxy: Arc::new(
                HostedProxyState::load(global_dir.join("hosted_proxy_state.json")).unwrap_or_else(
                    |e| {
                        eprintln!("Failed to load hosted proxy state: {e}");
                        std::process::exit(1);
                    },
                ),
            ),
    ...
    rt.block_on(async {
        ...
        let state = Arc::new(ServerState {
            ...
            hosted_proxy: Arc::new(
                HostedProxyState::load(global_dir.join("hosted_proxy_state.json")).unwrap_or_else(
                    |e| {
                        eprintln!("Error loading hosted proxy state: {e}");
                        std::process::exit(1);
                    },
                ),
            ),
