...
fn cmd_bootstrap() {
    ...
    rt.block_on(async move {
        ...
        let state = Arc::new(ServerState {
            ...
            hosted_proxy: crate::hosted_proxy::HostedProxyState::load(
                global_dir.join("hosted_proxy").join("registrations.json"),
                "bootstrap-hosted-proxy-secret",
            )
            .unwrap_or_else(|e| {
                eprintln!("Failed to initialize hosted proxy state: {e}");
                std::process::exit(1);
            }),
