...
fn ensure_secret_file(secret_path: &Path, label: &str, verbose: bool) {
    if secret_path.exists() {
        if verbose {
            println!("  {} secret already configured.", capitalize_label(label));
        }
        return;
    }
    ...
    if let Ok(mut f) = fs::File::open("/dev/urandom") {
        if f.read_exact(&mut buf).is_ok() {
            ...
            if fs::write(secret_path, &secret).is_ok() {
                ...
                if verbose {
                    println!("  Generated {} secret at {}", label, secret_path.display());
                    if label == "webhook" {
