...
use githubclaw::hosted_proxy::{FileHostedProxyStateStore, HostedProxyService};
...
async fn test_webhook_to_queue_roundtrip() {
    ...
    let state = Arc::new(ServerState {
        ...
        hosted_proxy: Mutex::new(HostedProxyService::new(
            FileHostedProxyStateStore::new(
                tmp.path().join("hosted_proxy").join("installations.json"),
            ),
            secret,
        )),
